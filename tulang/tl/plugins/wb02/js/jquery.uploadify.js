...
(function ($) {
    ...
    function wmzEditOnSelectNextFun(file) {
        ...
        var _nameArr = file.name.toString().toLocaleLowerCase().split(".");
        ...
        itemData = {
            ...
            'fileName': fileName,
            'fileSize': fileSize,
            suffix: _nameArr[_nameArr.length - 1]
        };
    ...
    function wmzEditOnSelectRecursiveFun(file, keys, objects, index) {
        var self = this;
        index = index || 0;
        var key = keys[index];
        var queuedFile = objects[key];
        if (queuedFile) {
            var _file = $('#' + queuedFile.id);
            if (queuedFile.uploaded != true && queuedFile.name == file.name && _file.length) {
                _file.remove();
                self.cancelUpload(queuedFile.id);
                self.queueData.filesReplaced++;
                wmzEditOnSelectRecursiveFun.call(self, file, keys, objects, ++index);
            } else {
                wmzEditOnSelectRecursiveFun.call(self, file, keys, objects, ++index);
            }
        } else {
            //结束
            wmzEditOnSelectNextFun.call(self, file);
        }
    }
    ...
    var handlers = {
        ...
        onDialogClose: function (filesSelected, filesQueued, queueLength) {
            ...
            if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
                if (this.queueData.filesErrored > 0) {
                    ...
                    layerAlert("温馨提示", this.queueData.errorMsg);
                }
            }
        ...
        //        if (queuedFile.uploaded != true && queuedFile.name == file.name && _file.length) {
        //
        //            var replaceQueueItem = confirm('文件名为 "' + file.name + '" 已经在队列中，是否进行替换？');
        //            if (!replaceQueueItem) {
        //                //取消
        ...
        onSelect: function (file) {
            ...
            wmzEditOnSelectRecursiveFun.call(this, file, keys, this.queueData.files);
        },
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\n你选择的文件数量，超过了设置的上传限制 (' + errorMsg + ').';
                        } else {
                            this.queueData.errorMsg += '\n你选择的文件数量，超过了队列设置的数量 (' + settings.queueSizeLimit + ').';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n你选择的文件 "' + file.name + '" 超过了大小限制 (' + settings.fileSizeLimit + 'KB).';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n你选择的文件 "' + file.name + '" 是一个空文件.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n你选择的文件 "' + file.name + '" 是不支持的上传类型 (' + settings.fileTypeDesc + ').';
                        break;
                }
            }
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('一个文件名称 "' + file.name + '" 已经存在在服务器上.\n你确定要替换吗?');
